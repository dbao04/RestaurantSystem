...
# --------------------------------------------------------------------------
def _tl_doanh_thu(kq, tham_so, boi_canh):
    c = kq["chi_so"]
    return (
        f"Doanh thu {_nhan_khoang(kq)}: **{tien(c['doanh_thu'])}**\n"
        f"- Số đơn: {nguyen(c['so_don'])} · Lượt khách: {nguyen(c['so_khach'])}\n"
        f"- Giá trị trung bình mỗi đơn: {tien(c['gia_tri_don_tb'])}"
    )


def _tl_so_sanh(kq, tham_so, boi_canh):
    n, t, d = kq["ky_nay"], kq["ky_truoc"], kq["thay_doi"]
    return (
        f"So sánh {_nhan_khoang(kq)} với {kq['khoang_truoc']['nhan']} "
        f"({kq['khoang_truoc']['tu']} → {kq['khoang_truoc']['den']}):\n"
        f"- Doanh thu: {tien(n['doanh_thu'])} so với {tien(t['doanh_thu'])} "
        f"→ **{_mui_ten(d['doanh_thu'])}**\n"
        f"- Số đơn: {nguyen(n['so_don'])} so với {nguyen(t['so_don'])} "
        f"→ {_mui_ten(d['so_don'])}\n"
        f"- Lợi nhuận gộp: {tien(n['loi_nhuan_gop'])} so với {tien(t['loi_nhuan_gop'])} "
        f"→ {_mui_ten(d['loi_nhuan_gop'])}"
    )
...
def _tl_so_don(kq, tham_so, boi_canh):
    ...
    return (
        ...
        f"bán ra {nguyen(c['so_mon'])} phần món, doanh thu {tien(c['doanh_thu'])}."
    )
...
def _tl_loi_nhuan(kq, tham_so, boi_canh):
    c = kq["chi_so"]
    return (
        f"Kết quả kinh doanh {_nhan_khoang(kq)}:\n"
        f"- Doanh thu: {tien(c['doanh_thu'])}\n"
        f"- Chi phí nguyên liệu (giá vốn): {tien(c['chi_phi_nguyen_lieu'])}\n"
        f"- **Lợi nhuận gộp: {tien(c['loi_nhuan_gop'])}** "
        f"(biên {phan_tram(c['bien_loi_nhuan'])})\n\n"
        f"_Lưu ý: đây là lợi nhuận gộp, chưa trừ lương, mặt bằng và chi phí vận hành._"
    )


def _tl_gia_tri_don_tb(kq, tham_so, boi_canh):
    c = kq["chi_so"]
    return (
        f"{_nhan_khoang(kq)}, trung bình mỗi hóa đơn **{tien(c['gia_tri_don_tb'])}** "
        f"({nguyen(c['so_don'])} đơn / {tien(c['doanh_thu'])})."
    )
...
def _tl_top_mon(kq, tham_so, boi_canh):
    ...
    return (
        ...
        f"**{dau['name_mon']}** với {nguyen(dau['so_luong'])} phần, "
        f"doanh thu {tien(dau['doanh_thu'])}."
    )
...
def _tl_ton_kho(kq, tham_so, boi_canh):
    b = kq.get("bang") or []
    if kq.get("loc_theo"):
        if not b:
            return f"Không tìm thấy nguyên liệu **{kq['loc_theo']}** trong kho."
        r = b[0]
        return (f"**{r['ten_nl']}** còn **{nguyen(r['ton'])} {r.get('ten_dvt') or ''}** "
                f"trong kho (định mức tối thiểu {nguyen(r['dinh_muc_min'])}).")
    return f"Tình trạng tồn kho hiện tại ({len(b)} nguyên liệu cần chú ý nhất):"


def _tl_nguyen_lieu_sap_het(kq, tham_so, boi_canh):
    b = kq.get("bang") or []
    if not b:
        return "Tốt — hiện **không có nguyên liệu nào** dưới định mức hay sắp cạn."
    return (
        f"Có **{len(b)} nguyên liệu** cần nhập thêm (dưới định mức, "
        f"hoặc còn đủ dùng chưa tới 3 ngày):"
    )


def _tl_lo_sap_het_han(kq, tham_so, boi_canh):
    b = kq.get("bang") or []
    if not b:
        return "Không có lô hàng nào hết hạn trong 14 ngày tới."
    gap = [r for r in b if (r.get("con_lai") is not None and float(r["con_lai"]) <= 3)]
    them = f" Trong đó **{len(gap)} lô cần xử lý gấp** (còn ≤ 3 ngày)." if gap else ""
    return f"Có **{len(b)} lô hàng** sắp hết hạn trong 14 ngày tới.{them}"


def _tl_hieu_suat_nv(kq, tham_so, boi_canh):
    b = kq.get("bang") or []
    if not b:
        return f"Không có dữ liệu phục vụ trong {_nhan_khoang(kq)}."
    d = b[0]
    return (
        f"Hiệu suất nhân viên {_nhan_khoang(kq)} — dẫn đầu là **{d['ten']}** "
        f"với {nguyen(d['so_don'])} đơn, doanh thu {tien(d['doanh_thu'])}."
    )
...
def _tl_gio_cao_diem(kq, tham_so, boi_canh):
    ...
    dinh = max(gio, key=lambda r: float(r["doanh_thu"] or 0))
    cau = (f"Khung giờ cao điểm {_nhan_khoang(kq)} là **{int(dinh['gio'])}h** "
           f"({nguyen(dinh['so_don'])} đơn, {tien(dinh['doanh_thu'])}).")
    if thu:
        dinh_thu = max(thu, key=lambda r: float(r["doanh_thu"] or 0))
...
BO_SINH = {
    "hoi_doanh_thu": _tl_doanh_thu,
    "hoi_so_sanh_doanh_thu": _tl_so_sanh,
    "hoi_so_don": _tl_so_don,
    "hoi_luot_khach": _tl_luot_khach,
    "hoi_loi_nhuan": _tl_loi_nhuan,
    "hoi_gia_tri_don_tb": _tl_gia_tri_don_tb,
    "hoi_top_mon": _tl_top_mon,
    "hoi_mon_ban_cham": _tl_mon_ban_cham,
    "hoi_ton_kho": _tl_ton_kho,
    "hoi_nguyen_lieu_sap_het": _tl_nguyen_lieu_sap_het,
    "hoi_lo_sap_het_han": _tl_lo_sap_het_han,
    "hoi_hieu_suat_nhan_vien": _tl_hieu_suat_nv,
...
GOI_Y_TIEP = {
    ...
    "quan_ly": [
        "Doanh thu hôm nay bao nhiêu?", "Top 10 món bán chạy tháng này",
        "Nguyên liệu nào sắp hết?", "So sánh doanh thu tuần này với tuần trước",
        "Khung giờ nào đông khách nhất?", "Dự báo khách ngày mai",
        "Còn mấy bàn trống?", "Lô hàng nào sắp hết hạn?",
    ],
}
...
def sinh(y_dinh_ma: str, ket_qua: dict, tham_so: dict, boi_canh: dict,
    ...
    if y_dinh_ma == "khong_hieu":
        ung_vien = [t for t in (du_doan or {}).get("top", [])[:3]]
        cau_hoi_gan = [yd.Y_DINH[u["y_dinh"]]["mo_ta"]
                       for u in ung_vien if u["y_dinh"] in yd.Y_DINH]
        van_ban = ("Xin lỗi, mình chưa hiểu ý bạn. Mình chỉ trả lời được các câu hỏi "
                   "về nhà hàng: thực đơn, giá món, đặt bàn, khuyến mãi, giao hàng…")
        if cau_hoi_gan:
            van_ban += "\n\nCó phải bạn muốn hỏi về: " + \
                       ", ".join(f"_{c}_" for c in cau_hoi_gan[:2]) + "?"
        return {"van_ban": van_ban, "goi_y": goi_y, "khong_hieu": True}
